...
RSpec.describe 'Get recipe', :vcr do
  describe "happy path" do
    it 'retrieves learning resources for a given country' do
      ...
      expect((parsed[:data]).keys).to eq([:id, :type, :attributes]) #checks that unnecessary information is NOT present
      expect((parsed[:data][:attributes]).keys).to eq([:country, :video, :images]) # only these keys, nothing more
      ...
      expect(parsed[:data][:attributes]).not_to have_key(:likes)
    end
  end
